            ...
            @if($allsettings->display_social_login == 1)
              ...
              <form action="{{ route('login') }}" method="POST" id="login_form" class="@if($allsettings->display_social_login == 0) py-3 @endif">
                ...
                <div class="d-flex flex-wrap justify-content-between">
                  ...
                  <a class="nav-link-inline font-size-sm" href="{{ URL::to('/guest') }}">{{ __('Check out as guest user?') }}</a>

                  @endif

                  <a class="nav-link-inline font-size-sm" href="{{ URL::to('/forgot') }}">{{ __('Forgot password?') }}</a>
